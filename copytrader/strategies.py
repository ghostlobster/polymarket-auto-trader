...
PRESETS: dict[str, StrategyPreset] = {
    "mirror": StrategyPreset(
        name="mirror", order_type="market", notional_scale=1.0,
        max_age_secs=60, max_slippage=0.05, resolves_in_min_hours=0.0,
        description="Highest fidelity. Same notional as leader (capped). Market orders. Tolerates slippage.",
    ),
    "scaled_market": StrategyPreset(
        name="scaled_market", order_type="market", notional_scale=0.01,
        max_age_secs=120, max_slippage=0.03, resolves_in_min_hours=24.0,
        description="Balanced default. 1% of leader notional, market fills, skips imminent resolutions.",
    ),
    "scaled_limit": StrategyPreset(
        name="scaled_limit", order_type="limit", notional_scale=0.01,
        max_age_secs=120, max_slippage=0.01, resolves_in_min_hours=24.0,
        description="Best price, may miss fills. Limit at leader's fill price; tight slippage.",
    ),
    "conservative": StrategyPreset(
        name="conservative", order_type="limit", notional_scale=0.005,
        max_age_secs=90, max_slippage=0.01, resolves_in_min_hours=168.0,
        description="Low risk. Half-percent of leader notional, only on markets that resolve >7d out.",
    ),
    "shadow": StrategyPreset(
        name="shadow", order_type="none", notional_scale=0.0,
        max_age_secs=120, max_slippage=1.0, resolves_in_min_hours=0.0,
...
def apply_preset(
    ...
    if ts > 0:
        ...
        if age > preset.max_age_secs:
            return CopyDecision(
                skip=True, reason=f"stale:{int(age)}s>{preset.max_age_secs}s",
                expected_copy=False,
            )
    ...
    if preset.resolves_in_min_hours > 0 and market_resolves_at is not None:
        ...
        if delta_h < preset.resolves_in_min_hours:
            return CopyDecision(
                skip=True, reason=f"resolves_in_{delta_h:.1f}h<{preset.resolves_in_min_hours}h",
                expected_copy=False,
            )
    ...
    if preset.order_type == "none":
        return CopyDecision(
            skip=True, reason="shadow_mode", expected_copy=False,
        )
    ...
    if size_usdc < 5.0:
        return CopyDecision(
            skip=True, reason=f"size<{5.0}_after_caps:{size_usdc:.2f}",
            expected_copy=False,
        )
    ...
    if preset.order_type == "market":
        return CopyDecision(
            skip=False, order_type="market", size_usdc=size_usdc,
            limit_price=None, expected_copy=True,
        )
    if preset.order_type == "limit":
        ...
        return CopyDecision(
            skip=False, order_type="limit", size_usdc=size_usdc,
            limit_price=round(limit_price, 4), expected_copy=True,
        )
