...
class Database:
    ...
    async def get_open_positions(self) -> list[Position]:
        async with self.conn.execute("SELECT * FROM positions WHERE closed_at IS NULL") as cur:
            rows = await cur.fetchall()
    ...
    async def get_paper_positions(self, wallet: str | None = None, open_only: bool = False):
        ...
        async with self.conn.execute(f"SELECT * FROM paper_positions{where}", args) as cur:
            rows = await cur.fetchall()
    ...
    async def get_web_user_by_id(self, user_id: int) -> dict | None:
        async with self.conn.execute("SELECT * FROM web_users WHERE id=?", (user_id,)) as cur:
            row = await cur.fetchone()
        return dict(row) if row else None
